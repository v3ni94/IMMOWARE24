...
Übergänge: `unanswered -> acknowledged | answered | no_response_needed`; `acknowledged -> answered | awaiting_reply | reply_received`; `answered -> awaiting_reply | reply_received`; `awaiting_reply -> reply_received`; `reply_received -> answered | awaiting_reply`. Ein Übergang auf `acknowledged` oder `answered` setzt voraus, dass eine `mail_drafts`-Zeile im Status `sent_verified` mit `case_id` existiert oder dass der Gmail-Abgleich eine direkt in Gmail gesendete Nachricht (Label `SENT`) im Thread des Vorgangs importiert hat, deren Empfänger (to, cc) einen eingehenden Absender des Vorgangs enthält (direkte Gmail-Antwort ohne Hub-Entwurf, Quelle `gmail`). Gesendete Nachrichten im Thread ohne diesen Empfänger (Weiterleitung an Handwerker, Rückfrage an Kollegen) werden nur als `forwarded` verknüpft und stoppen keine Uhr. Ein nur angeforderter Versand (`sent_requested`) verändert die Kommunikationsdimension nicht.
