...
13. learning_runs (Migration 2026_09_21_000001, Modul Learning, Änderungsvermerk 21.09.2026): kein Herkunftsblock, kein Bezug zu Immoware24-Spiegeldaten. Ein Lauf der Lernphase Immoware24 erkundet lesend über WebDAV, CardDAV, CalDAV oder bereits erfasste Dateiexporte (Spalte kind) die aktuelle Struktur, hält Rohbefunde und Vergleich zum vorigen erfolgreichen Lauf gleicher Art als JSON (facts_json, diff_json) und verweist optional auf einen Vorschlag in mail_ai_suggestions (ai_suggestion_id). previous_run_id ist bewusst kein Fremdschlüssel (Selbstbezug, siehe Migration).
14. mail_playbooks und mail_playbook_matches (Migration 2026_09_21_000002, Modul Playbooks, Änderungsvermerk 21.09.2026): mail_playbooks ist mandantengebunden mit Soft Delete (Herkunft eigene Wissensbasis, kein Bezug zu Immoware24), eine Prozessvorlage je Kategorie (case_type) und Version, nur status active wird beim Vergleich herangezogen. mail_playbook_matches protokolliert jeden Abgleich eines Vorgangs (case_id restrictOnDelete wie bei mail_action_plans) mit den Vorlagen, unabhängig vom Ergebnis, als Grundlage für die Verbesserung der Vorlagen.
