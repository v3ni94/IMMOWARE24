...
| `queue_depth` | Gauge | Queue-Tiefe je Queue (Label `queue`), minütlich gesetzt durch `hub:sync:queue-depth` (Scheduler, `SyncSchedule`) |
...
Das Gauge `queue_depth` setzt der Scheduler minütlich über `php artisan hub:sync:queue-depth` (Änderungsvermerk 12.09.2026): bei Redis-Treiber `LLEN` plus `ZCARD` der `:delayed`- und `:reserved`-Mengen je Queue aus `hub.core.queues` über die Redis-Connection der Queue-Connection, bei Datenbank-Treiber oder nicht erreichbarem Redis Zählung in der Tabelle `jobs` je Queue. Admin-Dashboard und `SyncMetrics::summary()` lesen das Gauge (Summe über alle Queues); `hub:sync:queue-depth --json` zeigt die Werte je Queue.
...
`HealthService::queue()` misst die Queue-Tiefe seit dem Abschlusslauf 12.09.2026 treiberabhängig: bei `QUEUE_CONNECTION=redis` per `PING`, `LLEN` je Queue und `ZCARD` der `:delayed`- und `:reserved`-Mengen (`depth`, `depth_by_queue`), ein Verbindungsfehler ist down; bei Datenbank-Treiber über die Tabelle `jobs`. `oldest_job_age_seconds` steht nur beim Datenbank-Treiber zur Verfügung, bei Redis bleibt der Wert null; Alarm A5 nutzt dann `queue_depth` (Gauge, Abschnitt 3) und den Worker-Heartbeat. `failed_jobs` (Datenbank) und `dlq_open` sind unabhängig vom Treiber korrekt.
