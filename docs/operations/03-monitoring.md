...
Weitere Betriebskennzahlen aus der Datenbank: `remote_requests` (Tabelle, Statuscodes, Latenzen, Aufbewahrung 90 Tage), `sync_runs`, `dlq_items`, `write_operations` je Status, `conflicts` offen, `audit_logs` letzte Verankerung. Redis: `LLEN queues:high` bis `queues:low`, `php artisan queue:monitor redis:high,redis:default,redis:sync,redis:write,redis:documents,redis:low --max=5000` (bereits Healthcheck des Worker-Containers).
...
| A6 Scheduler stumm | kein `sync_runs`-Eintrag seit 20 Minuten trotz aktiver Connection | hoch | operator | Scheduler-Prozess prüfen (`schedule:work` Container oder systemd-Timer) |
...
`HealthService::queue()` misst die Queue-Tiefe über die Tabelle `jobs`. Produktion nutzt `QUEUE_CONNECTION=redis`, dort ist die Tabelle leer und `depth` sowie `oldest_job_age_seconds` sind immer 0. `failed_jobs` (Datenbank) und `dlq_open` sind korrekt. Bis der Health-Check den Redis-Treiber abfragt, liefert der Alarm A5 seine Daten aus `queue:monitor` und `LLEN`. Zuständig: Entwicklung Modul Api und Sync. Datenintegrität ist nicht betroffen, nur die Sichtbarkeit.
